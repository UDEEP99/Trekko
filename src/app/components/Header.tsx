...
function NavDropdown({
  ...
  const gradients = {
    amber: "from-teal-400 to-[#00BCD4]",
    cyan: "from-cyan-400 to-[#00BCD4]",
  };
              ...
              items.map((trip, i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 text-left">
                    <p className="text-[13px] font-semibold text-ink dark:text-white truncate group-hover:text-[#00BCD4] transition-colors">
                      {trip.destination}
                    </p>
                  ...
                  <ChevronRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-[#00BCD4] transition-colors flex-shrink-0" />
                </motion.button>
              ))
            )}
          </div>
...
export default function Header({
  ...
  if (session?.user) {
    return (
      <div className="flex items-center gap-2">
        ...
        {/* ─── Divider ─── */}
        <div className="w-px h-6 bg-ink/10 dark:bg-white/20 mx-0.5" />
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <Link 
              href="/profile" 
              className="text-sm font-medium text-white/90 hover:text-white inline-flex items-center gap-1.5 transition-colors"
            >
              <User className="w-3.5 h-3.5 text-[#00BCD4]" />
        ...
        <motion.button
          ...
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className="flex items-center gap-1.5 px-3.5 py-2 rounded-full text-xs font-semibold text-white/70 bg-white/5 border border-white/10 hover:text-red-400 hover:border-red-400/50 hover:bg-red-500/10 transition-all duration-200 shadow-sm"
        >
  ...
  return (
    <motion.button
      ...
      whileHover={{ scale: 1.05 }}
      whileTap={{ scale: 0.95 }}
      className="bg-white/10 backdrop-blur-md border border-white/20 text-white px-5 py-2 rounded-full font-bold shadow-md hover:bg-white/20 transition-colors duration-200 text-sm flex items-center gap-2"
    >
      <ThemeToggle />
      Login
    </motion.button>
