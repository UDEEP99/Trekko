...
/* ─── Floating Background Decor (Beach Theme) ─── */
function FloatingDecor() {
    const items = [
        { Icon: Sun, x: "8%", y: "10%", size: 36, delay: 0, duration: 8, color: "text-amber-400/25" },
        { Icon: Cloud, x: "72%", y: "6%", size: 40, delay: 2, duration: 12, color: "text-cyan-300/15" },
        { Icon: TreePalm, x: "92%", y: "30%", size: 34, delay: 1, duration: 10, color: "text-emerald-400/20" },
        { Icon: Cloud, x: "18%", y: "40%", size: 30, delay: 4, duration: 14, color: "text-sky-300/12" },
        { Icon: TreePalm, x: "4%", y: "70%", size: 28, delay: 5, duration: 11, color: "text-emerald-300/15" },
        { Icon: Plane, x: "60%", y: "4%", size: 22, delay: 1.5, duration: 9, color: "text-orange-300/15" },
    ];

    return (
        <div className="fixed inset-0 pointer-events-none -z-10 overflow-hidden">
            {/* Theme-aware gradient blobs */}
            <div className="decor-blob decor-blob-cyan absolute -top-40 -right-40 w-[600px] h-[600px]" style={{ animation: "tropicalFloat 12s ease-in-out infinite" }} />
            <div className="decor-blob decor-blob-gold absolute top-1/3 -left-40 w-[500px] h-[500px]" style={{ animation: "tropicalFloat 15s ease-in-out infinite 3s" }} />
            <div className="decor-blob decor-blob-mint absolute -bottom-40 right-1/4 w-[450px] h-[450px]" style={{ animation: "tropicalFloat 18s ease-in-out infinite 5s" }} />
            <div className="decor-blob decor-blob-cyan absolute top-2/3 left-1/3 w-[350px] h-[350px]" style={{ animation: "tropicalFloat 18s ease-in-out infinite 7s" }} />

            {/* Animated ocean wave at bottom */}
            <div className="absolute bottom-0 left-0 right-0 h-[80px] opacity-30">
                <svg viewBox="0 0 1440 120" preserveAspectRatio="none" className="w-full h-full">
                    <path d="M0,80 C360,120 720,40 1080,80 C1260,100 1380,60 1440,80 L1440,120 L0,120 Z"
                        fill="rgba(0,188,212,0.15)" style={{ animation: "oceanWave1 8s ease-in-out infinite" }} />
                    <path d="M0,90 C360,60 720,100 1080,90 C1260,70 1380,95 1440,90 L1440,120 L0,120 Z"
                        fill="rgba(0,188,212,0.10)" style={{ animation: "oceanWave2 6s ease-in-out infinite" }} />
                </svg>
            </div>
            ...
            {[...Array(6)].map((_, i) => (
                <div key={`b${i}`} className="bubble" style={{
                    left: `${8 + i * 15}%`, width: `${3 + (i % 3) * 2}px`, height: `${3 + (i % 3) * 2}px`,
                    animationDuration: `${8 + i * 3}s`, animationDelay: `${i * 2}s`,
                }} />
            ))}
...
function CategoryIcon({ category, time }: { category?: string; time: string }) {
    ...
    if (cat.includes("food") || cat.includes("restaurant") || cat.includes("dining") || cat.includes("eat"))
        return <Utensils className="w-4 h-4 text-ink dark:text-white" />;
    if (cat.includes("sightsee") || cat.includes("photo") || cat.includes("view"))
        return <Camera className="w-4 h-4 text-ink dark:text-white" />;
    if (cat.includes("adventure") || cat.includes("hike") || cat.includes("trek") || cat.includes("outdoor"))
        return <Mountain className="w-4 h-4 text-ink dark:text-white" />;
    if (cat.includes("culture") || cat.includes("museum") || cat.includes("temple") || cat.includes("heritage"))
        return <Globe className="w-4 h-4 text-ink dark:text-white" />;
    if (cat.includes("nightlife") || cat.includes("music") || cat.includes("party") || cat.includes("bar"))
        return <Music className="w-4 h-4 text-ink dark:text-white" />;
    if (cat.includes("romantic") || cat.includes("couple") || cat.includes("date"))
        return <Heart className="w-4 h-4 text-ink dark:text-white" />;
    if (cat.includes("hotel") || cat.includes("relax") || cat.includes("spa") || cat.includes("resort"))
        return <Hotel className="w-4 h-4 text-ink dark:text-white" />;
    // fallback based on time of day
    if (time === "Morning") return <Sun className="w-4 h-4 text-ink dark:text-white" />;
    if (time === "Afternoon") return <Compass className="w-4 h-4 text-ink dark:text-white" />;
    return <Moon className="w-4 h-4 text-ink dark:text-white" />;
}
            ...
            <div className="splash-content">
                ...
                <h1 className={`splash-headline ${playfair.className}`}>
                    {words.map((word, i) => (
                        <motion.span
                            ...
                            className={word === "Trekko" ? "text-transparent bg-clip-text bg-gradient-to-r from-[#FFB347] via-[#00BCD4] to-[#69F0AE]" : ""}
                        >
                            {word}{" "}
                        </motion.span>
                    ))}
                ...
                <motion.div variants={splashWordVariants}>
                    <motion.button
                        ...
                        whileHover={{ scale: 1.06, boxShadow: "0 0 40px rgba(0, 188, 212, 0.5), 0 0 80px rgba(255, 179, 71, 0.3)" }}
                        whileTap={{ scale: 0.96 }}
                    >
...
const DESTINATION_IMAGES = [
    ...
    "https://images.unsplash.com/photo-1552832860-cfaf474cacf9?q=80&w=2000&auto=format&fit=crop",
...
export default function Home() {
    ...
    // ── Persist form state to sessionStorage so it survives profile navigation ──
    useEffect(() => {
        try {
            const saved = sessionStorage.getItem('trekko_dash_state');
            if (saved) {
                ...
                if (s.travelStyle) setTravelStyle(s.travelStyle);
                if (s.itinerary) setItinerary(s.itinerary);
                if (s.selectedDestinationImage) setSelectedDestinationImage(s.selectedDestinationImage);
                if (s.activeDayIndex !== undefined) setActiveDayIndex(s.activeDayIndex);
            }
        } catch { /* ignore */ }
    }, []);
    ...
    useEffect(() => {
        try {
            sessionStorage.setItem('trekko_dash_state', JSON.stringify({
                destination, days, travelers, budget, travelStyle,
                itinerary, selectedDestinationImage, activeDayIndex,
            }));
        } catch { /* ignore */ }
    }, [destination, days, travelers, budget, travelStyle, itinerary, selectedDestinationImage, activeDayIndex]);

    // Scroll to results when itinerary is ready and fully rendered
    useEffect(() => {
        if (!loading && itinerary && resultsRef.current) {
    ...
    return (
        <AnimatePresence mode="wait">
            {!hasStarted ? (
                ...
                <motion.div
                    key="main-app"
                    className="relative min-h-screen overflow-hidden dashboard-bg"
                ...
                >
                    ...
                    <nav className="dashboard-nav" style={{ background: "transparent", backdropFilter: "blur(12px)" }}>
                        <div className="max-w-6xl mx-auto px-6 h-full flex items-center justify-between w-full">
                            <motion.div
                            ...
                            >
                                <Link href="/" className="flex items-center gap-1 cursor-pointer">
                                    ...
                                    <span className="text-lg font-bold tracking-widest italic text-white" style={{ fontFamily: "'Playfair Display', serif" }}>
                                        Trekko
                                    </span>
                                </Link>
                            </motion.div>
                                        ...
                                        >
                                            {cloudantSyncing ? (
                                                <motion.span
                                                    ...
                                                    className="w-3.5 h-3.5 border-2 border-[#E8C56A] border-t-transparent rounded-full inline-block"
                                                />
                    ...
                    <section className="relative w-full min-h-[60vh] overflow-hidden flex flex-col items-center justify-center pt-24">
                        ...
                        <div className="absolute bottom-6 right-6 z-20 bg-black/50 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex items-center gap-2">
                            <MapPin className="w-4 h-4 text-[#E8C56A]" />
                        ...
                        <div className="absolute inset-0 z-10 bg-gradient-to-b from-[#052840]/60 via-[#083A55]/70 to-[#052840]" />
                        ...
                        <div className="relative z-20 w-full max-w-4xl px-4 pb-16">
                            ...
                            >
                                <form
                                    ...
                                    className="w-full max-w-3xl form-glass p-6 md:p-8 shadow-2xl mx-auto perspective-container"
                                >
                                    <AnimatePresence>
                                    ...
                                    <div className="mb-6">
                                        <label className="text-sm dash-subtext font-medium px-1 mb-2 block">Where to?</label>
                                        <div ref={suggestionsRef} className="relative w-full">
                                            <div className="dash-input">
                                                ...
                                                <button
                                                    ...
                                                    type="button"
                                                    className={`absolute right-2 p-2 rounded-full transition-all duration-300 ${isListening
                                                            ? 'bg-red-500/20 text-red-500 animate-pulse shadow-[0_0_15px_rgba(239,68,68,0.4)]'
                                                            : 'text-ink/40 dark:text-white/40 hover:text-[#00BCD4] hover:bg-ink/5 dark:hover:bg-white/5'
                                                        }`}
                                            ...
                                            <AnimatePresence>
                                                {showSuggestions && suggestions.length > 0 && (
                                                    <motion.div
                                                        ...
                                                        style={{ transformOrigin: "top" }}
                                                        className="absolute z-50 left-0 right-0 top-[calc(100%+8px)] suggestion-dropdown"
                                                    >
                                                        {suggestions.map((s) => (
                                                            <button
                                                                key={s.id}
                                                            ...
                                                            >
                                                                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#FFB300]/15 to-[#00BCD4]/15 flex items-center justify-center flex-shrink-0">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="flex flex-col gap-2 w-full">
                                            <label className="text-sm dash-subtext font-medium px-1 mb-2 block">Number of Days</label>
                                            <div className="dash-input">
                                                <div className="input-icon">
                                                ...
                                                />
                                                <div className="absolute right-2 flex items-center gap-1 bg-ink/5 dark:bg-white/10 p-1 rounded-lg border border-ink/5 dark:border-white/10 shadow-inner">
                                                    <button
                                                        onClick={() => setDays(prev => Math.max(1, prev - 1))}
                                                        className="p-1.5 text-ink/40 dark:text-white/60 hover:text-ink dark:hover:text-white hover:bg-ink/5 dark:hover:bg-white/10 rounded-md transition-colors"
                                                        type="button"
                                                    >
                                                        <Minus className="w-4 h-4" />
                                                    </button>
                                                    <div className="w-[1px] h-4 bg-ink/10 dark:bg-white/20 mx-1"></div>
                                                    <button
                                                        onClick={() => setDays(prev => prev + 1)}
                                                        className="p-1.5 text-ink/40 dark:text-white/60 hover:text-[#FFB300] hover:bg-ink/5 dark:hover:bg-white/10 rounded-md transition-colors"
                                                        type="button"
                                                    >
                                        ...
                                        <div className="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                                            <div>
                                                <label className="text-sm dash-subtext font-medium px-1 mb-2 block">Who is traveling?</label>
                                                <div className="grid grid-cols-3 gap-3">
                                                    {[{ id: 'Solo', icon: User }, { id: 'Couple', icon: Users }, { id: 'Family', icon: Baby }].map((item) => (
                                                        <div
                                                            key={item.id}
                                                            onClick={() => setTravelers(item.id)}
                                                            className={`chip ${travelers === item.id ? 'chip-active' : ''}`}
                                                        >
                                            ...
                                            <div>
                                                <label className="text-sm dash-subtext font-medium px-1 mb-2 block">Budget</label>
                                                <div className="grid grid-cols-3 gap-3">
                                                    {[{ id: 'Backpacker', icon: Backpack }, { id: 'Moderate', icon: Wallet }, { id: 'Luxury', icon: Gem }].map((item) => (
                                                        <div
                                                            key={item.id}
                                                            onClick={() => setBudget(item.id)}
                                                            className={`chip ${budget === item.id ? 'chip-active' : ''}`}
                                                        >
                                                            <item.icon className="w-5 h-5" />
                                                            <span className="text-xs font-semibold">{item.id}</span>
                                                        </div>
                                        ...
                                        <div className="col-span-1 md:col-span-2 mt-4">
                                            <label className="text-sm dash-subtext font-medium px-1 mb-2 block">Travel Style</label>
                                                ...
                                                {[
                                                    ...
                                                    { id: 'Party', icon: PartyPopper, label: 'Party' }
                                                        ...
                                                        <div
                                                            ...
                                                            className={`chip ${travelStyle === item.id ? 'chip-active-cyan' : ''}`}
                                                        >
                                                        <item.icon className="w-5 h-5" />
                        ...
                        {itinerary && (
                            <section ref={resultsRef} className="px-6 pb-20">
                                <div className="max-w-3xl mx-auto">
                                    ...
                                    >
                                        <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold dash-heading italic">
                                            Your Trip to{" "}
                                            <span className="text-gradient">{destination}</span>
                                        </h2>
                                        <p className="mt-3 dash-subtext text-base">
                                    ...
                                    {moodBoardImages.length > 0 && (
                                        ...
                                        >
                                            <div className="mb-4">
                                                <h3 className="text-2xl sm:text-3xl font-bold dash-heading italic">Mood Board</h3>
                                                <p className="text-sm dash-subtext mt-1">
                                                    A visual preview of your upcoming journey.
                                                </p>
                                            </div>
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                                <div className="overflow-hidden rounded-2xl min-h-[260px] md:min-h-[340px] relative">
                                                    <Image
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    {moodBoardImages.slice(1, 5).map((imageUrl, imageIndex) => (
                                                        <div key={`${imageUrl}-${imageIndex}`} className="overflow-hidden rounded-2xl h-32 sm:h-40 md:h-[162px] relative">
                                                            <Image
                                                                src={imageUrl}
                                    ...
                                    <div className="mt-8">
                                            ...
                                            {itinerary.map((dayPlan, index) => {
                                                ...
                                                return (
                                                    <button
                                                        key={dayPlan.day}
                                                        type="button"
                                                        onClick={() => { setSlideDirection(index > activeDayIndex ? 'next' : 'prev'); setActiveDayIndex(index); }}
                                        ...
                                        {/* Card with flanking prev/next buttons */}
                                        <div className="relative mt-5 flex items-start gap-3">
                                            {/* Left: Previous button */}
                                            <div className="hidden md:flex items-center pt-20 flex-shrink-0">
                                                {activeDayIndex > 0 ? (
                                                    <motion.button
                                                        type="button"
                                                        ...
                                                        whileTap={{ scale: 0.92 }}
                                                        className="w-12 h-12 rounded-full flex items-center justify-center day-nav-btn !p-0 !rounded-full shadow-lg"
                                                        title="Previous Day"
                                                    >
                                            ...
                                            {/* Center: Day card with swipe animation */}
                                            <div className="flex-1 min-w-0 overflow-hidden" style={{ perspective: '1200px' }}>
                                                <AnimatePresence mode="popLayout" custom={slideDirection}>
                                                    {itinerary[activeDayIndex] && (
                                                        <motion.div
                                                            ...
                                                            className="itinerary-card group relative z-10 focus-within:z-50"
                                                        ...
                                                        >
                                                            ...
                                                            <div className="flex items-center gap-3 mb-5">
                                                                ...
                                                                <div>
                                                                    <h3 className="font-bold dash-heading">Day {itinerary[activeDayIndex].day}</h3>
                                                                    <p className="text-sm text-[#FFB300] font-medium">{itinerary[activeDayIndex].theme}</p>
                                                                </div>
                                                                <div className="ml-auto dash-subtext text-xs font-medium">
                                                                    {activeDayIndex + 1} / {itinerary.length}
                                                                </div>
                                                            </div>
                                                                    ...
                                                                    <div key={activity.id || aIdx} className="flex flex-row justify-between items-start gap-2">
                                                                        ...
                                                                        {activity.isCustom && activity.id && (
                                                                            <button
                                                                                ...
                                                                                className="p-2 dash-subtext hover:text-red-400 hover:bg-red-500/10 rounded-md transition-colors cursor-pointer mt-2"
                                                            ...
                                                            <div className="mt-6 hotel-cta">
                                                                <div className="flex items-center gap-3.5">
                                                                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#FFB300]/20 to-[#00BCD4]/20 flex items-center justify-center flex-shrink-0">
                                                                        <Hotel className="w-5 h-5 text-[#FFB300]" />
                                                                    </div>
                                                                    <div>
                                                                        <h4 className="font-semibold dash-heading text-sm sm:text-base">Find hotels in {destination}</h4>
                                                                        <p className="text-xs dash-subtext mt-0.5">Discover great places to stay near your activities.</p>
                                                                    </div>
                                                                </div>
                                                                <a
                                                                    ...
                                                                    target="_blank"
                                                                    rel="noopener noreferrer"
                                                                    className="hotel-cta-btn w-full sm:w-auto"
                                                                >
                                                                    Search Hotels
                                                                </a>
                                                            </div>
                                                            ...
                                                            <div className="flex justify-between w-full mt-6 md:hidden">
                                                                <div>
                                                                    {activeDayIndex > 0 && (
                                                                        <button type="button" onClick={handlePrevDay} className="day-nav-btn">
                                                                ...
                                                                <div>
                                                                    {activeDayIndex < itinerary.length - 1 && (
                                                                        <button type="button" onClick={handleNextDay} className="day-nav-btn day-nav-btn-primary">
                                            ...
                                            <div className="hidden md:flex items-center pt-20 flex-shrink-0">
                                                {activeDayIndex < itinerary.length - 1 ? (
                                                    <motion.button
                                                        ...
                                                        whileHover={{ scale: 1.12 }}
                                                        whileTap={{ scale: 0.92 }}
                                                        className="w-12 h-12 rounded-full flex items-center justify-center day-nav-btn day-nav-btn-primary !p-0 !rounded-full shadow-lg"
                                                        title="Next Day"
                                                    >
                                    ...
                                    <div className="mt-14 space-y-10">
                                        <div>
                                            <h3 className="text-xl font-bold dash-heading mb-4 flex items-center gap-2">
                                                <Hotel className="w-5 h-5 text-[#FFB300]" />
                                            ...
                                            <div className="flex flex-row overflow-x-auto gap-4 hide-scrollbar px-2 py-4 -mx-2">
                                                {[
                                                    ...
                                                    <a
                                                        ...
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className="flex-shrink-0 w-64 dash-card group block"
                                                    >
                                                        ...
                                                        <div className="p-4">
                                                            <h4 className="font-bold dash-heading text-base truncate">{hotel.name}</h4>
                                                            <p className="text-xs dash-subtext mt-1.5">{hotel.desc}</p>
                                                        </div>
                                                    </a>
                                                ))}
                                            </div>
                                        ...
                                        <div>
                                            <h3 className="text-xl font-bold dash-heading mb-4 flex items-center gap-2">
                                                <Map className="w-5 h-5 text-[#00BCD4]" />
                                            ...
                                            <div className="flex flex-row overflow-x-auto gap-4 hide-scrollbar px-2 py-4 -mx-2">
                                                {[
                                                    ...
                                                    >
                                                        <div className="h-36 overflow-hidden relative">
                                                            <Image src={tour.img} alt={tour.name} fill className="object-cover group-hover:scale-110 transition-transform duration-700" />
                                                            <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                                                        </div>
                                                        <div className="p-4">
                                                            <h4 className="font-bold dash-heading text-base truncate">{tour.name}</h4>
                                                            <p className="text-xs dash-subtext mt-1.5">{tour.desc}</p>
                                                        </div>
                                                    </a>
                                                ))}
                                            </div>
                                    ...
                                    >
                                        <button
                                            ...
                                            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-[#FFB300]/25 text-[#FFB300] font-medium text-sm hover:bg-[#FFB300]/5 transition-colors duration-200"
                                        >
                                            <Sparkles className="w-4 h-4" />
                                            Plan another trip
                                        </button>
                    ...
                    <footer className="py-8 text-center text-sm dash-subtext border-t border-ink/5 dark:border-white/5">
